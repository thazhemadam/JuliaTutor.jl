...
function user_experience(syntax_error, incorrect_answer)
    if syntax_error
        println(Crayon( foreground = :red, italics = true ), "🔧 Sorry it appears there is a syntax error in your code. Please try again!")
    ...
    if correct
        println(Crayon( foreground = :green, italics = true ), "✔ Good job!" )
    end
